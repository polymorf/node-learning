...
pki_db.run("CREATE TABLE privatekeys (uuid TEXT PRIMARY KEY, name TEXT UNIQUE, size INTEGER NOT NULL, key BLOB NOT NULL, modulus BLOB NOT NULL);");
pki_db.run("CREATE TABLE certificates (uuid TEXT PRIMARY KEY, keyuuid TEXT NOT NULL, name TEXT UNIQUE, cert BLOB NOT NULL, modulus BLOB NOT NULL, FOREIGN KEY(keyuuid) REFERENCES privatekeys(uuid) );");
...
function savePrivateKey( uuid, name, key, modulus, callback) {
